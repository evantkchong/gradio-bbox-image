...
@document()
class BBoxImage(StreamingInput, Component):
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A PIL BBoxImage, numpy array, path or URL for the default value that BBoxImage component is going to take. If callable, the function will be called whenever the app loads to set the initial value of the component.
        ...
        if streaming and self.sources != ["webcam"]:
            raise ValueError(
                "BBoxImage streaming only available if sources is ['webcam']. Streaming not supported with multiple sources."
            )
